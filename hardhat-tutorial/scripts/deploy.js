...
async function main() {
  ...
  const cryptoDevTokensContract = await ethers.getContractFactory("CryptoDevToken"); //this is the contract that we are providing it some factory libraries to use. In other words it promises us to give it to us. In return to promise it provides us with an instance of the contract do that we can call the contract over it
